...
bst %>% fit(x_train, y_train, epochs = 2, batch_size = 128)

### Verify that test AUC is same as what's reported in the paper
bst_pred_test <- as.vector(predict(bst, x_test))
roc(y_test, bst_pred_test) # 0.9115
ci.auc(roc(y_test, bst_pred_test), conf.level = 0.95) # 95% CI: 0.9089-0.9141
...
res
write.csv(res, paste0("./Analysis/ESI_lr.csv"), row.names=TRUE)
...
colnames(res) <- set
rownames(res) <- set
write.csv(res, paste0("./Analysis/race_lr.csv"), row.names=TRUE)
...
colnames(res) <- set
rownames(res) <- set
write.csv(res, paste0("./Analysis/insurance_lr.csv"), row.names=TRUE)
...
colnames(res) <- set
rownames(res) <- set
write.csv(res, paste0("./Analysis/previousdispo_lr.csv"), row.names=TRUE)
...
plot(coldiff, pdiff, main="LR", xlab="Perturbation", ylab="Probability change",
     ylim=c(-0.2, 0.2))
abline(h=0)
